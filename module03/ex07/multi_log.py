...
def create_mono_log_model(zipcode: int, x_train, y_train, y_test, column_len) -> MyLR:
...
def main():
	...
	models, preds = [], []
	for zipcode in range(zipcode_amount):
		model = create_mono_log_model(zipcode, x_train, y_train, y_test, column_len)
		models.append(model)
	...
	for idx, feature in enumerate(features):
		plt.title(f'Zipcode plotted against {feature.capitalize()}')
		plt.xlabel(feature.capitalize())
		plt.ylabel('Zipcode')
		plt.yticks(range(0, zipcode_amount), zipcode_names)
		column = x_test[:, idx]
